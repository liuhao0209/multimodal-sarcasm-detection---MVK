...
class ScaledDotProductAttention_image_text(nn.Module):
    ''' Scaled Dot-Product Attention '''
    ...
    def forward(self, q, k, v, mask=None):
        ...
        attn = self.softmax(attn) # att是注意力分数
        attn = self.dropout(attn).transpose(1,2) # output是注意力机制的结果
        output = torch.bmm(attn, v)
...
class PositionwiseFeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.w_2(nn.LeakyReLU(self.w_1(output)))
...
class PositionWiseFeedForward_change_dim(nn.Module):
    ...
    def __init__(self, dim, ff_dim, outdim, dropout):
        super().__init__()
        self.fc1 = nn.Linear(dim, ff_dim)
        self.fc2 = nn.Linear(ff_dim, outdim)
        self.layer_norm = LayerNorm(outdim)
        self.LkReLU = nn.LeakyReLU(0.01)
        self.dropout = nn.Dropout(dropout)
    def forward(self, x):
        residual = x
        # (B, S, D) -> (B, S, D_ff) -> (B, S, D)
        fc2_out = self.fc2( self.LkReLU( self.fc1(x) ) )
        fc2_out = self.dropout(fc2_out)
        out = self.layer_norm(fc2_out)
        return out
...
if __name__ =='__main__':
    input=torch.randn(50,1,512)
    sa = ScaledDotProductAttention(temperature = 512, attn_dropout=0.1)
    output=sa(input,input,input)
    print(output[0].shape, output[1].shape)

    MHA = MultiHeadAttention(n_head=6, d_model=512, d_k=512, d_v=512, dropout=0.1, is_regu=False)
    output = MHA(input, input, input)
    print(output[0].shape, output[1].shape)
    PF = PositionwiseFeedForward(d_in = 1024, d_hid = 152)